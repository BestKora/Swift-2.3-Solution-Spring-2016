...
class TweetTableViewController: UITableViewController, UITextFieldDelegate
{
    ...
    private func printDatabaseStatistics() {
        moc?.performBlock {
            if let results = try? self.moc!.executeFetchRequest(NSFetchRequest(entityName: "TweetM")) {
                print("\(results.count) TweetMs")
            }
            // a more efficient way to count objects ...
            let searchCount = try! self.moc!.countForFetchRequest(NSFetchRequest(entityName: "SearchTerm"))
            print("\(searchCount) SearchTerms")
            let mensionCount = try! self.moc!.countForFetchRequest(NSFetchRequest(entityName: "Mension"))
